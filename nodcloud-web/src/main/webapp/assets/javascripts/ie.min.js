!function (a) {
...
}(this), document.createElement("canvas").getContext || !function () {
    function a() {
    ...
    function b(a, b) {
        ...
        return function () {
            return a.apply(b, c.concat(O.call(arguments)))
        }
    }
    ...
    function k(a) {
        var b = a.indexOf("(", 3), c = a.indexOf(")", b + 1), d = a.substring(b + 1, c).split(",");
    ...
    function n(a) {
        var b, c, d, e, f, g;
        if (e = parseFloat(a[0]) / 360 % 360, 0 > e && e++, f = m(l(a[1]), 0, 1), g = m(l(a[2]), 0, 1), 0 == f)b = c = d = g; else {
            var h = .5 > g ? g * (1 + f) : g + f - g * f, i = 2 * g - h;
    ...
    function p(a) {
        ...
        var b, c = 1;
        if (a = String(a), "#" == a.charAt(0))b = a; else if (/^rgb/.test(a)) {
    ...
    function q(a) {
        ...
        return W[a] = {"style": c.fontStyle || V.style, "variant": c.fontVariant || V.variant, "weight": c.fontWeight || V.weight, "size": c.fontSize || V.size, "family": c.fontFamily || V.family}
    }
    ...
    function r(a, b) {
        ...
        return c.size = "number" == typeof a.size ? a.size : -1 != a.size.indexOf("px") ? f : -1 != a.size.indexOf("em") ? e * f : -1 != a.size.indexOf("%") ? e / 100 * f : -1 != a.size.indexOf("pt") ? f / .75 : e, c.size *= .981, c
    }
    ...
    function v(a, b, c, d) {
        a.currentPath_.push({"type": "bezierCurveTo", "cp1x": b.x, "cp1y": b.y, "cp2x": c.x, "cp2y": c.y, "x": d.x, "y": d.y}), a.currentX_ = d.x, a.currentY_ = d.y
    }

    function w(a, b) {
        var c = p(a.strokeStyle), d = c.color, e = c.alpha * a.globalAlpha, f = a.lineScale_ * a.lineWidth;
    ...
    function x(a, b, c, d) {
        var e = a.fillStyle, f = a.arcScaleX_, g = a.arcScaleY_, h = d.x - c.x, i = d.y - c.y;
        if (e instanceof B) {
            var j = 0, k = {"x": 0, "y": 0}, l = 0, m = 1;
            if ("gradient" == e.type_) {
                var n = e.x0_ / f, o = e.y0_ / g, q = e.x1_ / f, r = e.y1_ / g, s = y(a, n, o), t = y(a, q, r), u = t.x - s.x, v = t.y - s.y;
                j = 180 * Math.atan2(u, v) / Math.PI, 0 > j && (j += 360), 1e-6 > j && (j = 0)
            } else {
                var s = y(a, e.x0_, e.y0_);
                k = {"x": (s.x - c.x) / h, "y": (s.y - c.y) / i}, h /= f * M, i /= g * M;
                var w = G.max(h, i);
            ...
            var N = p(a.fillStyle), O = N.color, P = N.alpha * a.globalAlpha;
    ...
    function y(a, b, c) {
        var d = a.m_;
        return{"x": M * (b * d[0][0] + c * d[1][0] + d[2][0]) - N, "y": M * (b * d[0][1] + c * d[1][1] + d[2][1]) - N}
    }
    ...
    Y.clearRect = function () {
        ...
        var c = y(this, a, b);
        this.currentPath_.push({"type": "moveTo", "x": c.x, "y": c.y}), this.currentX_ = c.x, this.currentY_ = c.y
    }, Y.lineTo = function (a, b) {
        var c = y(this, a, b);
        this.currentPath_.push({"type": "lineTo", "x": c.x, "y": c.y}), this.currentX_ = c.x, this.currentY_ = c.y
    }, Y.bezierCurveTo = function (a, b, c, d, e, f) {
        var g = y(this, e, f), h = y(this, a, b), i = y(this, c, d);
        v(this, h, i, g)
    }, Y.quadraticCurveTo = function (a, b, c, d) {
        var e = y(this, a, b), f = y(this, c, d), g = {"x": this.currentX_ + 2 / 3 * (e.x - this.currentX_), "y": this.currentY_ + 2 / 3 * (e.y - this.currentY_)}, h = {"x": g.x + (f.x - this.currentX_) / 3, "y": g.y + (f.y - this.currentY_) / 3};
        v(this, g, h, f)
    }, Y.arc = function (a, b, c, d, e, f) {
        c *= M;
        var g = f ? "at" : "wa", h = a + J(d) * c - N, i = b + I(d) * c - N, j = a + J(e) * c - N, k = b + I(e) * c - N;
        h != j || f || (h += .125);
        var l = y(this, a, b), m = y(this, h, i), n = y(this, j, k);
        this.currentPath_.push({"type": g, "x": l.x, "y": l.y, "radius": c, "xStart": m.x, "yStart": m.y, "xEnd": n.x, "yEnd": n.y})
        ...
        var n = y(this, b, c), o = [], p = 10, q = 10;
        if (o.push(" <g_vml_:group", ' coordsize="', M * p, ",", M * q, '"', ' coordorigin="0,0"', ' style="width:', p, "px;height:", q, "px;position:absolute;"), 1 != this.m_[0][0] || this.m_[0][1] || 1 != this.m_[1][1] || this.m_[1][0]) {
            var r = [];
            r.push("M11=", this.m_[0][0], ",", "M12=", this.m_[1][0], ",", "M21=", this.m_[0][1], ",", "M22=", this.m_[1][1], ",", "Dx=", H(n.x / M), ",", "Dy=", H(n.y / M), "");
            var s = n, t = y(this, b + d, c), u = y(this, b, c + e), v = y(this, b + d, c + e);
            s.x = G.max(s.x, t.x, u.x, v.x), s.y = G.max(s.y, t.y, u.y, v.y), o.push("padding:0 ", H(s.x / M), "px ", H(s.y / M), "px 0;filter:progid:DXImageTransform.Microsoft.Matrix(", r.join(""), ", sizingmethod='clip');")
        } else o.push("top:", H(n.y / M), "px;left:", H(n.x / M), "px;");
        o.push(' ">', '<g_vml_:image src="', a.src, '"', ' style="width:', M * d, "px;", " height:", M * e, 'px"', ' cropleft="', f / l, '"', ' croptop="', g / m, '"', ' cropright="', (l - f - h) / l, '"', ' cropbottom="', (m - g - i) / m, '"', " />", "</g_vml_:group>"), this.element_.insertAdjacentHTML("BeforeEnd", o.join(""))
    }, Y.stroke = function (a) {
        for (var b = 10, c = 10, d = 5e3, e = {"x": null, "y": null}, f = {"x": null, "y": null}, g = 0; g < this.currentPath_.length; g += d) {
            var h = [];
            h.push("<g_vml_:shape", ' filled="', !!a, '"', ' style="position:absolute;width:', b, "px;height:", c, 'px;"', ' coordorigin="0,0"', ' coordsize="', M * b, ",", M * c, '"', ' stroked="', !a, '"', ' path="');
            for (var i = g; i < Math.min(g + d, this.currentPath_.length); i++) {
                i % d == 0 && i > 0 && h.push(" m ", H(this.currentPath_[i - 1].x), ",", H(this.currentPath_[i - 1].y));
                var j, k = this.currentPath_[i];
                switch (k.type) {
                    case"moveTo":
                        j = k, h.push(" m ", H(k.x), ",", H(k.y));
                        break;
                    case"lineTo":
                        h.push(" l ", H(k.x), ",", H(k.y));
                        break;
                    case"close":
                        h.push(" x "), k = null;
                        break;
                    case"bezierCurveTo":
                        h.push(" c ", H(k.cp1x), ",", H(k.cp1y), ",", H(k.cp2x), ",", H(k.cp2y), ",", H(k.x), ",", H(k.y));
                        break;
                    case"at":
                    case"wa":
                        h.push(" ", k.type, " ", H(k.x - this.arcScaleX_ * k.radius), ",", H(k.y - this.arcScaleY_ * k.radius), " ", H(k.x + this.arcScaleX_ * k.radius), ",", H(k.y + this.arcScaleY_ * k.radius), " ", H(k.xStart), ",", H(k.yStart), " ", H(k.xEnd), ",", H(k.yEnd))
                }
                k && ((null == e.x || k.x < e.x) && (e.x = k.x), (null == f.x || k.x > f.x) && (f.x = k.x), (null == e.y || k.y < e.y) && (e.y = k.y), (null == f.y || k.y > f.y) && (f.y = k.y))
            }
            h.push(' ">'), a ? x(this, h, e, f) : w(this, h), h.push("</g_vml_:shape>"), this.element_.insertAdjacentHTML("beforeEnd", h.join(""))
        }
    }, Y.fill = function () {
        this.stroke(!0)
        ...
        var g = this.m_, h = 1e3, i = 0, j = h, k = {"x": 0, "y": 0}, l = [], m = r(q(this.font), this.element_), n = s(m), o = this.element_.currentStyle, p = this.textAlign.toLowerCase();
        switch (p) {
            case"left":
            case"center":
            case"right":
        ...
        l.push('<g_vml_:line from="', -i, ' 0" to="', j, ' 0.05" ', ' coordsize="100 100" coordorigin="0 0"', ' filled="', !f, '" stroked="', !!f, '" style="position:absolute;width:1px;height:1px;">'), f ? w(this, l) : x(this, l, {"x": -i, "y": 0}, {"x": j, "y": m.size});
        var u = g[0][0].toFixed(3) + "," + g[1][0].toFixed(3) + "," + g[0][1].toFixed(3) + "," + g[1][1].toFixed(3) + ",0,0", v = H(t.x / M) + "," + H(t.y / M);
        ...
        return this.textMeasureEl_.innerHTML = "", this.textMeasureEl_.style.font = this.font, this.textMeasureEl_.appendChild(c.createTextNode(a)), {"width": this.textMeasureEl_.offsetWidth}
    }, Y.clip = function () {
    }, Y.arcTo = function () {
    }, Y.createPattern = function (a, b) {
        return new C(a, b)
    }, B.prototype.addColorStop = function (a, b) {
        b = p(b), this.colors_.push({"offset": a, "color": b.color, "alpha": b.alpha})
    };
